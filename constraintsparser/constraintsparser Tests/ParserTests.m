...
{
    id result = [self.parser parse:@"a.width == b.height"];
...
- (void)testMultiplier
{
    id result = [self.parser parse:@"self.baseline >= test.hi.centerX * 2.3"];
...
{
    id result = [self.parser parse:@"self.one.two.baseline <= test.centerX * 2.3"];
...
- (void)testConstants
{
    id result = [self.parser parse:@"a12.baseline <= 10 + test.centerX * 0.5"];
